...
module.exports={
     ...
     * @returns {Promise.<*>}
     */
    async get(database,table,where,paras){
        //先查redis 中是否有
    ...
    async getOne(database,table,where,paras){
        ...
        if(id!==null)
            return await this.get(database,table,where,id);
     ...
     */
    async delete(database,table,where,id){
     ...
     */
    async multiGet(database,table,where,id){
        !(id instanceof Array) && (id=[id]);
        id=[...new Set(id)];
        let result=[];
        ...
        result=res.filter(v=>v);

        //如果redis查出的数量少，则从DB 中补充查询
        if(result.length<id.length){
            let tmp_id=id;
            result.forEach(v=>tmp_id.splice(tmp_id.indexOf(v.id),1));
            ...
            let db_res=await _module.findAll({ where:{ id:tmp_id }});
        ...
        result.forEach(v=>{
            promiseList.push((
                async (v)=>{
                    ...
                    if(id!==null){
                        if(to_field === 'id'){
                            v[paras]=await this.get(database,table,where,id);
                        }else{
